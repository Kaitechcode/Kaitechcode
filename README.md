# Kai Karube

Computer Engineering @ Columbia University ('29) · Yanai Tadashi Foundation Scholar

Kobe → Hamburg → London → Milan → New York

## World models & spatial technology

I've spent the last three years getting reality into computers by hand —
photogrammetry of a centuries-old Kyoto temple (Taizō-in), AR interfaces for
lunar vehicles, Gaussian-splat capture experiments. The question I keep coming
back to: **when do learned world models replace hand-built reconstruction — and
what does reconstruction still know that generation doesn't** (metric accuracy,
semantics you can trust)?

That's the thread connecting everything below: capture → reconstruction →
learned simulation.

## Now

- 🛰 **NASA SUITS** (Columbia Space Initiative, LTV team) — AR UI/UX for
  astronaut interfaces: designed the state color-coding system astronauts read
  at a glance, and built error-code → actionable-instruction translation.
  → [SUITS-MagicLeap-26](https://github.com/Kaitechcode/SUITS-MagicLeap-26)
- 🚗 **Summer 2026: PdM intern @ Recruit (Carsensor)** — new-business plan +
  working prototype (3D vehicle scanning, AI damage detection); grand prize at
  the final VP presentation
- 🧱 **Foundations, built in public** — daily DSA
  ([neetcode-submissions](https://github.com/Kaitechcode/neetcode-submissions)),
  C++ systems, Karpathy's *Zero to Hero* (micrograd → GPT)

## Find me

[LinkedIn](https://www.linkedin.com/in/kaikarube)
